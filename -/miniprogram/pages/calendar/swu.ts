...
bind_publish:function(e){    //点击某个具体的元素，获取到他在数组中的下标，根据下标，将该元素_id值传递给下一个页面
  ...
  Inplire: function(data) {
    var DateInputAction = data.detail.value.DateInputAction;
    var flag=0;
    var concent1='';
    for(let i=0;i<this.data.eventsArray.length;i++)
    {
      let datedata=this.data.eventsArray[i][1];
      if(datedata.includes(DateInputAction))
      {
         concent1=concent1+this.data.eventsArray[i][1]+'日期为'+this.data.eventsArray[i][0];
        flag=1;
      }
    }
    if(flag==0)
